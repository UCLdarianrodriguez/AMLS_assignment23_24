...
def roc_multiclass(class_probabilities,true_labels,model_name,name):
    ...
    Arguments:
        class_probabilities (numpy.ndarray): Predicted probabilities for each class.
        true_labels (numpy.ndarray): True labels for each sample.
    ...
    for i in range(9):
        ...
        fpr, tpr, _ = roc_curve(binary_labels, class_probabilities[:, i])
        roc_auc = auc(fpr, tpr)
    ...
    plt.title(f'ROC Curves {model_name} Predictions')
    plt.legend(loc='best')
